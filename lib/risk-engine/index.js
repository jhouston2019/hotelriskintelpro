...
const { analyzeCarrierBehavior } = require('../carrier-intelligence');
 ...
 * @returns {Promise<Object>} HotelRiskAnalysis
 */
async function analyzeHotelRisk(input) {
  ...
  // Step 7: Analyze carrier behavior (if carrier name provided)
  let carrierIntelligence = null;
  if (policyProfile.carrier && typeof analyzeCarrierBehavior === 'function') {
    try {
      carrierIntelligence = await analyzeCarrierBehavior(
        policyProfile,
        { ...hotelProfile, annualRevenue: financialProfile.annualGrossRevenue },
        lossRuns
      );
    } catch (error) {
      console.error('Carrier intelligence analysis failed:', error);
    }
  }
  ...
  return {
    ...
    // Carrier intelligence (if available)
    carrierIntelligence,
